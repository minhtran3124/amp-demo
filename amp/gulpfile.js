...
var compileExample = require('./tasks/compile-example');
var sitemap = require('./tasks/compile-sitemap');
var validateExample = require('./tasks/validate-example');
var createExample = require('./tasks/create-example');
var FileName = require('./tasks/lib/FileName');
var Metadata = require('./tasks/lib/Metadata');
...
gulp.task('serve', 'starts a local webserver (--port specifies bound port)',
  function() {
    ...
    var server = gls.static(paths.dist.dir, port);
...
gulp.task('create', 'create a new AMP example', function() {
  var title = argv.n || argv.name;
  var fileName = FileName.fromString(title);
  if (!fileName) {
    throwInvalidArgumentError('example name missing');
  }
  let examplePath;
  var dest = argv.d || argv.dest;
  var category = argv.c || argv.category;
  if (dest) {
...
function performChange(content) {
  var exampleFile = ExampleFile.fromPath(this.file.path);
  var canonical = config.host + exampleFile.url();
